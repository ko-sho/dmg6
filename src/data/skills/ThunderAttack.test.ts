import { ThunderAttack } from './ElementalAttackSkills';
import { describe, it, expect } from 'vitest';

describe('ThunderAttack スキル定義', () => {
  it('全体構造・レベル・パラメータが不変で正しい', () => {
    expect(ThunderAttack.category).toBe('weapon');
    expect(ThunderAttack.name).toBe('雷属性攻撃強化');
    // レベルごとのパラメータ
    const expected = [
    ...
    // レベル数
    expect(ThunderAttack.levels.length).toBe(expected.length);
    // 各レベルのパラメータ
    for (let i = 0; i < expected.length; i++) {
      expect(ThunderAttack.levels[i].level).toBe(expected[i].level);
      expect(ThunderAttack.levels[i].skillBonuses).toEqual(expected[i].skillBonuses);
    }
  });
});
